	...
	while(!reemplazada){
		...
		if(!pagina_en_memoria->usada){
			...
			if(pagina_en_memoria->modificada){
				...
				escribir_en_archivo(get_path_archivo(proceso->id), pagina_en_memoria);
			}

			t_list* contenido_archivo = leer_contenido_pagina_archivo(get_path_archivo(proceso->id), pagina);
...
void escribir_en_archivo(char* path, t_pagina* pagina) {
	...
	char* tabla_proceso = string_itoa(proceso->numero_tabla_primer_nivel);
...
void asignar_marco(t_pagina* pagina){
	...
	pagina->presencia = true;
	pagina->usada = true;
	t_list* contenido = leer_contenido_pagina_archivo(get_path_archivo(proceso->id), pagina);
